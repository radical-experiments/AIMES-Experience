...
#print unit[unit['Executing'].isnull()]
print '----------------------------------------------------------------'
...
for i in range(len(Tx_array)):
    if not isnan(Tx_array[i][1]) and not isnan(Tx_array[i][0]):
        avg_Tx += Tx_array[i][1] - Tx_array[i][0]
        cnt += 1
avg_Tx /= cnt
print "Avg Tx: %f, count: %d" % (avg_Tx, cnt)
...
print "Resource:%s;Num_Units:%d;Avg_Tx:%f;Tx:%f" % (pilot['resource'][0], pilot['num_units'][0], avg_Tx, Tx)
'''
Tq_array = list()
for i in range(len(pilot.index)):
