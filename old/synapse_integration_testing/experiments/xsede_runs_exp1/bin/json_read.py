...
def read_json(data_file):
    ...
    with open(data_file) as json_data:
        ...
        for i in range(len(d['pilot'])):
            ...
            tmp_pilot['resource'] = d['pilot'][i]['description']['resource'] 
...
pilot, unit = read_json(data_file)
print data_file
#print unit
#print pilot
...
for k in range(len(pilot.index)):
    if 'Failed' in pilot.columns and not isnan(pilot['Failed'][k]):
        pass

    else:
        Tx_unit = unit[unit['pilot'] == k]
        Tx_unit.index = range(len(Tx_unit.index))

        Tx_array = []
        for i in range(len(Tx_unit.index)):
            if not isnan(Tx_unit['PendingOutputStaging'][i]):
                Tx_array.append([Tx_unit['Executing'][i], Tx_unit['PendingOutputStaging'][i]])

        avg_Tx = 0.0
        for i in range(len(Tx_array)):
            avg_Tx += Tx_array[i][1] - Tx_array[i][0]
        avg_Tx /= len(Tx_array)
        #print "Avg Tx: %f" % avg_Tx
...
#                       print "Overlapping range. Extending, then breaking", Tx_array[i], Tx_interval[current_interval]
        ...
        #print "Tx: %f" % Tx
        print "Resource:%s;Num_Units:%d;Avg_Tx:%f;Tx:%f" % (pilot['resource'][k], pilot['num_units'][k], avg_Tx, Tx)
'''
Tq_array = list()
for i in range(len(pilot.index)):
